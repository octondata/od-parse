...
from od_parse.ocr.handwritten import (
...
class TestOCRModule(unittest.TestCase):
    ...
    @patch("cv2.cvtColor")
    def test_extract_handwritten_content_mocked(
        self, mock_cvtColor, mock_image_open, mock_image_to_string
    ):
    ...
    def test_preprocess_image_for_ocr(
        self, mock_erode, mock_dilate, mock_threshold, mock_blur, mock_cvtColor
    ):
