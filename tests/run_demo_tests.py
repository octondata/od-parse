...
if excel_file.exists():
    ...
    try:
        ...
        if not EXCEL_AVAILABLE:
            print(
                "✗ Excel processing not available. Install with: pip install od-parse[excel]"
            )
        else:
            ...
            json_result = excel_to_json(
                str(excel_file), output_file=str(OUTPUT_DIR / "excel_output.json")
            )
            print(f"✓ JSON saved to: {OUTPUT_DIR / 'excel_output.json'}")

            # Parse to Markdown
            md_result = excel_to_markdown(
                str(excel_file), output_file=str(OUTPUT_DIR / "excel_output.md")
            )
            ...
            for sheet_name, sheet_data in json_result["sheets"].items():
                print(
                    f"    - {sheet_name}: {sheet_data['row_count']} rows, {len(sheet_data['columns'])} columns"
                )
                ...
                for sheet_name in info.keys():
                    ...
                    try:
                        ...
                        result = processor.query(
                            str(excel_file), f'SELECT * FROM "{safe_name}" LIMIT 5'
                        )
