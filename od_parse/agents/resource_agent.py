...
class ResourceAgent:
    ...
    def __init__(
        ...
        logger.info(f"ResourceAgent initialized: CPU cores={self.cpu_count}, "
    ...
    def recommend_workers(
        ...
        estimated_memory_per_worker_mb: float = 200.0
    ) -> int:
        """
        ...
        if recommended < requested_workers:
            logger.warning(f"Reducing workers from {requested_workers} to {recommended} "
    ...
    def check_memory_available(self, required_mb: float) -> bool:
        ...
        if available < required_mb:
            logger.warning(f"Insufficient memory: required={required_mb:.0f}MB, "
                         f"available={available:.0f}MB")
            return False
        
        return True
    
    def recommend_image_quality(
        self,
        requested_dpi: int,
        page_count: int
    ) -> int:
        ...
        logger.warning(f"Reducing DPI from {requested_dpi} to {recommended_dpi} "
