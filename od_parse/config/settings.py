...
DEFAULT_CONFIG = {
    ...
    "vector_db": {
        "pgvector": {"default_dimension": 1536},
        "qdrant": {"default_host": "localhost", "default_port": 6333},
        "chroma": {"default_path": "./chroma_db"},
    },
    # System prompts
    "system_prompts": {
        ...
        with open(path, "r") as f:
            if path.suffix.lower() in [".yaml", ".yml"]:
                if YAML_AVAILABLE:
                    ...
                    logger.warning(
                        "PyYAML not installed. Please install it with: pip install pyyaml"
                    )
                    return {}
            else:
                return json.load(f)
...
def _load_from_env() -> Dict[str, Any]:
    ...
    if os.environ.get("ANTHROPIC_API_KEY"):
        _set_nested_dict(
            config, "api_keys.anthropic", os.environ.get("ANTHROPIC_API_KEY")
        )
    ...
    if os.environ.get("HUGGINGFACE_API_KEY"):
        _set_nested_dict(
            config, "api_keys.huggingface", os.environ.get("HUGGINGFACE_API_KEY")
        )
    ...
    if os.environ.get("OD_PARSE_OPENAI_API_KEY"):
        _set_nested_dict(
            config, "api_keys.openai", os.environ.get("OD_PARSE_OPENAI_API_KEY")
        )

    if os.environ.get("OD_PARSE_ANTHROPIC_API_KEY"):
        _set_nested_dict(
            config, "api_keys.anthropic", os.environ.get("OD_PARSE_ANTHROPIC_API_KEY")
        )

    if os.environ.get("OD_PARSE_COHERE_API_KEY"):
        _set_nested_dict(
            config, "api_keys.cohere", os.environ.get("OD_PARSE_COHERE_API_KEY")
        )
    ...
    if os.environ.get("OD_PARSE_HUGGINGFACE_API_KEY"):
        _set_nested_dict(
            ...
            os.environ.get("OD_PARSE_HUGGINGFACE_API_KEY"),
        )

    if os.environ.get("OD_PARSE_GOOGLE_API_KEY"):
        _set_nested_dict(
            config, "api_keys.google", os.environ.get("OD_PARSE_GOOGLE_API_KEY")
        )

    # API endpoints
    if os.environ.get("OD_PARSE_OPENAI_API_URL"):
        _set_nested_dict(
            config, "api_endpoints.openai", os.environ.get("OD_PARSE_OPENAI_API_URL")
        )
    ...
    if os.environ.get("OD_PARSE_ANTHROPIC_API_URL"):
        _set_nested_dict(
            ...
            os.environ.get("OD_PARSE_ANTHROPIC_API_URL"),
        )

    if os.environ.get("OD_PARSE_COHERE_API_URL"):
        _set_nested_dict(
            config, "api_endpoints.cohere", os.environ.get("OD_PARSE_COHERE_API_URL")
        )
    ...
    if os.environ.get("OD_PARSE_HUGGINGFACE_API_URL"):
        _set_nested_dict(
            ...
            os.environ.get("OD_PARSE_HUGGINGFACE_API_URL"),
        )

    if os.environ.get("OD_PARSE_GOOGLE_API_URL"):
        _set_nested_dict(
            config, "api_endpoints.google", os.environ.get("OD_PARSE_GOOGLE_API_URL")
        )

    # VLM models
    if os.environ.get("OD_PARSE_QWEN_MODEL"):
        _set_nested_dict(
            config, "vlm_models.qwen", os.environ.get("OD_PARSE_QWEN_MODEL")
        )

    if os.environ.get("OD_PARSE_CLAUDE_MODEL"):
        _set_nested_dict(
            config, "vlm_models.claude", os.environ.get("OD_PARSE_CLAUDE_MODEL")
        )

    if os.environ.get("OD_PARSE_GEMINI_MODEL"):
        _set_nested_dict(
            config, "vlm_models.gemini", os.environ.get("OD_PARSE_GEMINI_MODEL")
        )
    ...
    if os.environ.get("OD_PARSE_OPENAI_EMBEDDING_MODEL"):
        _set_nested_dict(
            ...
            os.environ.get("OD_PARSE_OPENAI_EMBEDDING_MODEL"),
        )
    ...
    if os.environ.get("OD_PARSE_COHERE_EMBEDDING_MODEL"):
        _set_nested_dict(
            ...
            os.environ.get("OD_PARSE_COHERE_EMBEDDING_MODEL"),
        )
    ...
    if os.environ.get("OD_PARSE_HUGGINGFACE_EMBEDDING_MODEL"):
        _set_nested_dict(
            ...
            os.environ.get("OD_PARSE_HUGGINGFACE_EMBEDDING_MODEL"),
        )
