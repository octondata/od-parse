...
class DocumentClassifier:
    ...
    def classify_document(self, parsed_data: Dict[str, Any]) -> DocumentAnalysis:
        ...
        try:
            ...
            suggestions = self._generate_suggestions(
                document_type, confidence, all_signals
            )
    ...
    def _analyze_document_structure(
        self, text: str, tables: List, forms: List
    ) -> Dict[str, float]:
    ...
    def _analyze_content_semantics(self, text: str) -> Dict[str, float]:
        ...
        financial_matches = sum(
            1 for phrase in financial_phrases if phrase in text_lower
        )
        if financial_matches > 0:
            semantic_scores["financial_semantics"] = min(
                financial_matches / len(financial_phrases), 1.0
            )
        ...
        if legal_matches > 0:
            semantic_scores["legal_semantics"] = min(
                legal_matches / len(legal_phrases), 1.0
            )
    ...
    def _determine_classification(
        ...
        for signal_name, score in signals.items():
            ...
            if signal_name.startswith("pattern_"):
                ...
                try:
                    ...
                    type_scores[doc_type] = (
                        type_scores.get(doc_type, 0.0) + score * 2.0
            ...
            # Handle semantic and structural signals
                ...
                type_scores[DocumentType.TAX_FORM_1040] = (
                    type_scores.get(DocumentType.TAX_FORM_1040, 0.0) + score
                )
            elif "financial" in signal_name:
                type_scores[DocumentType.BANK_STATEMENT] = (
                    type_scores.get(DocumentType.BANK_STATEMENT, 0.0) + score
                )
            elif "legal" in signal_name:
                type_scores[DocumentType.LEGAL_CONTRACT] = (
                    type_scores.get(DocumentType.LEGAL_CONTRACT, 0.0) + score
                )
            elif "medical" in signal_name:
                type_scores[DocumentType.MEDICAL_RECORD] = (
                    type_scores.get(DocumentType.MEDICAL_RECORD, 0.0) + score
                )
    ...
    def _extract_key_indicators(
        ...
        if doc_type in [DocumentType.TAX_FORM_1040, DocumentType.TAX_FORM_W2]:
            ...
            account_match = re.search(
                r"account\s+(?:number|#)?\s*:?\s*(\d+)", text.lower()
            )
            if account_match:
            ...
            invoice_match = re.search(
                r"invoice\s+(?:number|#)?\s*:?\s*([A-Za-z0-9-]+)", text.lower()
            )
            if invoice_match:
    ...
    def _generate_suggestions(
        self, doc_type: DocumentType, confidence: float, signals: Dict[str, float]
    ) -> List[str]:
        ...
        if doc_type == DocumentType.TAX_FORM_1040:
            suggestions.append(
                "Consider extracting tax fields: income, deductions, tax liability"
            )
            ...
            suggestions.append(
                "Extract invoice details: number, date, amounts, line items"
            )
        ...
        if len(signals) < 3:
            suggestions.append(
                "Limited classification signals - consider additional analysis"
            )
