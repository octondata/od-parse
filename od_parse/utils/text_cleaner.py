...
def clean_extracted_text(
    ...
    if remove_ocr_artifacts:
        ...
        if len(cleaned) < before_len:
            ...
            logger.debug(f"Removed OCR artifacts (reduced by {before_len - len(cleaned)} chars)")
    ...
    if remove_special_chars:
        ...
        if len(cleaned) < before_len:
            ...
            logger.debug(f"Removed special characters (reduced by {before_len - len(cleaned)} chars)")
    ...
    if min_alpha_ratio is not None:
        ...
        if is_garbage:
            logger.warning(f"Text appears to be garbage (alpha ratio: {alpha_ratio:.2f})")
    ...
    return {
        ...
        'reduction_percent': ((len(original_text) - len(cleaned)) / len(original_text) * 100) if original_text else 0
    }
...
def extract_valid_names(text: str, min_alpha_ratio: float = 0.7) -> Optional[str]:
    ...
    if cleaned['is_garbage']:
        logger.warning(f"Name appears to be garbage (alpha ratio: {cleaned['alpha_ratio']:.2f}): {text[:50]}")
        return None
...
def get_text_quality_score(text: str) -> Dict[str, Any]:
    ...
    space_score = min(space_ratio / 0.15, 1.0) if space_ratio < 0.3 else 0.5  # Target 15% spaces
    special_score = 1.0 - min(special_char_ratio / 0.1, 1.0)  # Penalize >10% special chars
    word_length_score = 1.0 if 3 <= avg_word_length <= 8 else 0.5
    cid_score = 1.0 if cid_count == 0 else max(0.0, 1.0 - (cid_count / 100))
    
    overall_score = (alpha_score + space_score + special_score + word_length_score + cid_score) / 5
