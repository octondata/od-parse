...
class AdvancedTableExtractor:
    ...
    def _find_tables_from_lines(
        self, h_lines: List[Dict], v_lines: List[Dict], page: int
    ) -> List[Dict]:
        """
        ...
        h_coords = [
            line.get("bbox", [0, 0, 0, 0])[1] for line in h_lines
        ]  # y-coordinates
        v_coords = [
            line.get("bbox", [0, 0, 0, 0])[0] for line in v_lines
        ]  # x-coordinates
        ...
        for i in range(len(h_coords) - 1):
            for j in range(len(v_coords) - 1):
                ...
                internal_h_lines = [
                    line
                    for line in h_lines
                    if x1 <= line.get("bbox", [0, 0, 0, 0])[0] <= x2
                    and x1 <= line.get("bbox", [0, 0, 0, 0])[2] <= x2
                    and y1 < line.get("bbox", [0, 0, 0, 0])[1] < y2
                ]

                internal_v_lines = [
                    line
                    for line in v_lines
                    if y1 <= line.get("bbox", [0, 0, 0, 0])[1] <= y2
                    and y1 <= line.get("bbox", [0, 0, 0, 0])[3] <= y2
                    and x1 < line.get("bbox", [0, 0, 0, 0])[0] < x2
                ]
                ...
                internal_h_coords = sorted(
                    set(
                        [line.get("bbox", [0, 0, 0, 0])[1] for line in internal_h_lines]
                    )
                )
                internal_v_coords = sorted(
                    set(
                        [line.get("bbox", [0, 0, 0, 0])[0] for line in internal_v_lines]
                    )
                )
                ...
                tables.append(
                    {"x1": x1, "y1": y1, "x2": x2, "y2": y2, "rows": rows, "cols": cols}
                )
    ...
    def _find_aligned_text_blocks(self, blocks: List[Dict], page: int) -> List[Dict]:
        ...
        for block in blocks:
            y_center = (
                block.get("bbox", [0, 0, 0, 0])[1] + block.get("bbox", [0, 0, 0, 0])[3]
            ) / 2
    ...
    def _find_cells(self, table: Table, pdf_data: Dict) -> List[TableCell]:
        ...
        for row in range(table.rows):
            for col in range(table.cols):
                ...
                cell = TableCell(row=row, col=col, x1=x1, y1=y1, x2=x2, y2=y2)
    ...
    def _extract_cell_content(
        self, cell: TableCell, table: Table, pdf_data: Dict
    ) -> TableCell:
        """
