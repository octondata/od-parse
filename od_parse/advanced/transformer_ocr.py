...
class TransformerOCR:
    ...
    def __init__(
        ...
        self.use_gpu = (
            use_gpu and torch.cuda.is_available() if HAVE_TRANSFORMERS else False
        )
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            self.logger.info(
                f"Loading handwritten text recognition model: {self.handwritten_model_name}"
            )
            self.handwritten_processor = TrOCRProcessor.from_pretrained(
                self.handwritten_model_name
            )
            ...
            self.logger.info(
                f"Loading printed text recognition model: {self.printed_model_name}"
            )
            self.printed_processor = TrOCRProcessor.from_pretrained(
                self.printed_model_name
            )
    ...
    def extract_text(
        ...
        """
        if not HAVE_TRANSFORMERS or (
            self.handwritten_model is None and self.printed_model is None
        ):
        ...
        try:
            ...
            processor = (
                self.handwritten_processor if is_handwritten else self.printed_processor
            )
    ...
    def _detect_and_correct_orientation(
        ...
        for angle in angles:
            ...
            if angle == 0:
                ...
                rotated = cv2.warpAffine(
                    gray, rotation_matrix, (width, height), flags=cv2.INTER_CUBIC
                )
        ...
        if best_angle != 0:
            ...
            rotated_image = cv2.warpAffine(
                image_array, rotation_matrix, (width, height), flags=cv2.INTER_CUBIC
            )
...
class HandwrittenTextRecognizer:
    ...
    def __init__(
        ...
        self.ocr_engine = (
            TransformerOCR() if use_transformers and HAVE_TRANSFORMERS else None
        )

        self.logger = logging.getLogger(__name__)

    def process_document(
        self, image_path: Union[str, Path], output_path: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for region in classified_regions:
                ...
                if self.ocr_engine:
                    ocr_result = self.ocr_engine.extract_text(
                        region_image, is_handwritten=is_handwritten
                    )
                ...
                region_result = {
                    ...
                    "text": extracted_text,
                    "confidence": (
                        ocr_result.get("confidence", 0.5) if self.ocr_engine else 0.5
                    ),
                }
            ...
            result = {
                ...
                "handwritten_count": sum(
                    1 for r in extracted_content if r["type"] == "handwritten"
                ),
            }
            ...
            return {"content": [], "method": "failed", "error": str(e)}
    ...
    def _analyze_layout(self, image: np.ndarray) -> List[Dict[str, Any]]:
        ...
        if not self.use_layout_analysis:
            ...
            return [{"bbox": [0, 0, 1, 1], "type": "unknown"}]  # Normalized coordinates
        ...
        for contour in contours:
            ...
            # Add region
            regions.append(
                {"bbox": bbox, "type": "unknown"}
            )  # Will be classified in next step

        # If no regions detected, use whole image
        if not regions:
            regions.append({"bbox": [0, 0, 1, 1], "type": "unknown"})
    ...
    def _classify_regions(
        ...
        for region in regions:
            ...
            if self.ocr_engine:
                ...
                gray = (
                    cv2.cvtColor(region_image, cv2.COLOR_BGR2GRAY)
                ...
                if stroke_widths:
                    variation = (
                        np.std(stroke_widths) / np.mean(stroke_widths)
    ...
    def _apply_context_correction(
        ...
        for region in regions:
            ...
            corrections = {
                "l": (
                    "1" if region["type"] == "handwritten" else "l"
                ),  # Common confusion in handwriting
