...
class VectorStorage:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.embedding_model_name = self.config.get(
            "embedding_model_name", get_config(f"embedding_models.{embedding_provider}")
        )
        self.embedding_dimension = self.config.get(
            "embedding_dimension", get_config(f"vector_db.pgvector.default_dimension")
        )
        ...
        self.api_key = self.config.get(
            "api_key",
            get_config(f"api_keys.{embedding_provider}")
            or os.environ.get(f"{embedding_provider.upper()}_API_KEY"),
        )
    ...
    def _init_embedding_model(self):
        ...
        if self.embedding_model == "openai":
            try:
                ...
                self.logger.info(
                    f"Initialized OpenAI embedding model: {self.embedding_model_name}"
                )
            except ImportError:
                self.logger.error(
            ...
            try:
                ...
                self.logger.info(
                    f"Initialized HuggingFace embedding model: {self.embedding_model_name}"
                )
            except ImportError:
                self.logger.error(
                    "sentence-transformers package not installed. Please install it with: pip install sentence-transformers"
                )
            ...
            try:
                ...
                if not self.api_key:
                    self.logger.warning(
                        "Cohere API key not provided. Embeddings will not work."
                    )
                    return

                self.embedding_client = cohere.Client(self.api_key)
                self.logger.info(
                    f"Initialized Cohere embedding model: {self.embedding_model_name}"
                )
            except ImportError:
                self.logger.error(
                    "Cohere package not installed. Please install it with: pip install cohere"
                )
            ...
            if custom_module_path and custom_class_name:
                try:
                    spec = importlib.util.spec_from_file_location(
                        "custom_embeddings", custom_module_path
                    )
                    custom_module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(custom_module)

                    custom_class = getattr(custom_module, custom_class_name)
                    self.embedding_client = custom_class(
                        **self.config.get("custom_embedding_params", {})
                    )
                    self.logger.info(
                        f"Initialized custom embedding model from {custom_module_path}"
                    )
                except Exception as e:
                    self.logger.error(f"Error loading custom embedding model: {str(e)}")
            else:
                self.logger.error(
                    "Custom embedding model specified but module path or class name not provided"
                )
    ...
    def create_embeddings(self, parsed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            "document": {
                ...
                "metadata": {
                    "file_name": parsed_data.get("metadata", {}).get(
                        "file_name", "unknown"
                    ),
        ...
        try:
            if self.embedding_model == "openai":
                ...
                response = self.embedding_client.embed(
                    texts=[text], model=self.embedding_model_name
                )
    ...
    def _store_pgvector(self, embeddings: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.error(
                "psycopg2 module not installed. Please install it with: pip install psycopg2-binary"
            )
            return False
        ...
        try:
            with psycopg2.connect(self.connection_string) as conn:
                with conn.cursor() as cursor:
                    ...
                    doc_id = (
                        doc_metadata.get("file_name", "").replace(".", "_")
                        + "_"
                        + str(hash(str(doc_metadata)))
                    )

                    cursor.execute(
                        sql.SQL(
                            "INSERT INTO document_embeddings (id, file_name, embedding, metadata) VALUES (%s, %s, %s, %s)"
                        ),
                        (
                            doc_id,
                    ...
                    for chunk in chunk_embeddings:
                        ...
                        cursor.execute(
                            sql.SQL(
                                "INSERT INTO chunk_embeddings (id, document_id, text, embedding, metadata) VALUES (%s, %s, %s, %s, %s)"
                            ),
                            (
                                chunk_id,
                                doc_id,
            ...
            self.logger.error(
                f"Error storing embeddings in pgvector database: {str(e)}"
            )
            return False
