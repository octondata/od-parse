...
class LLaVANextEngine:
    ...
    def _initialize_llava_next(self) -> bool:
        ...
        if not config.is_feature_enabled("llava_next"):
            self.logger.info(
                "LLaVA-NeXT feature is disabled. Use config.enable_feature('llava_next') to enable."
            )
            return False

        if not config.is_feature_available("llava_next"):
            self.logger.warning(
                "LLaVA-NeXT dependencies not available. Install with: pip install od-parse[llava_next]"
            )
            return False
        ...
        try:
            ...
            self.logger.info(
                f"Loading LLaVA-NeXT model: {self.model_name} on {self.device}"
            )
    ...
    def _initialize_fallback(self):
        ...
        self.logger.info(
            "Using fallback document understanding (basic text extraction)"
        )
    ...
    def _understand_with_fallback(
        self, image: Image.Image, prompt: str, **kwargs
    ) -> Dict[str, Any]:
...
"""
    ...
    def _estimate_response_confidence(self, response: str) -> float:
        ...
        element_count = sum(
            1 for element in document_elements if element in response.lower()
        )
    ...
    def extract_structured_information(
        ...
        **kwargs,
        ...
        if information_type == "tables":
            prompt += (
                " Format your response to clearly separate each table and its contents."
            )
