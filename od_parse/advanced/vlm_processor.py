...
class VLMProcessor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.api_key = self.config.get(
            "api_key",
            get_config(f"api_keys.{api_provider}")
            or os.environ.get(f"{api_provider.upper()}_API_KEY"),
        )

        self.api_base = self.config.get(
            "api_base", get_config(f"api_endpoints.{api_provider}")
        )
        ...
        self.system_prompt = self.config.get(
            "system_prompt", get_config("system_prompts.document_analysis")
        )
    ...
    def _init_vlm_client(self):
        ...
        if self.model.startswith("qwen"):
            try:
                ...
                if not self.api_key:
                    self.logger.warning(
                        "API key not provided. VLM processing will not work."
                    )
                    return
            ...
            try:
                ...
                if not self.api_key:
                    self.logger.warning(
                        "Anthropic API key not provided. VLM processing will not work."
                    )
                    return
            ...
            try:
                ...
                if not self.api_key:
                    self.logger.warning(
                        "Google API key not provided. VLM processing will not work."
                    )
                    return
    ...
    def process_document_image(
        self, image_path: Union[str, Path], prompt: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _process_with_gemini(self, image: Image.Image, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {"model": self.model, "analysis": response.text}
    ...
    def enhance_parsing_results(
        self, parsed_data: Dict[str, Any], image_path: Union[str, Path]
    ) -> Dict[str, Any]:
        """
        ...
        if "error" in vlm_results:
            self.logger.warning(
                f"VLM processing error: {vlm_results['error']}. Using original parsing results."
            )
            parsed_data["vlm_analysis"] = {
                "status": "error",
                "message": vlm_results["error"],
            }
            return parsed_data
        ...
        try:
            ...
            structured_results = self.process_document_image(
    ...
    def extract_form_fields_with_vlm(
        self, image_path: Union[str, Path]
    ) -> Dict[str, Any]:
        """
        ...
        if "error" in vlm_results:
            self.logger.warning(
                f"VLM form field extraction error: {vlm_results['error']}"
            )
            return {}
