...
class NeuralTableExtractor:
    ...
    def __init__(
        ...
        self.device = (
            torch.device("cuda" if self.use_gpu else "cpu") if HAVE_DL_DEPS else None
        )
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            self.detection_processor = TableTransformerProcessor.from_pretrained(
                self.model_name
            )
            ...
            self.structure_processor = TableTransformerProcessor.from_pretrained(
            ...
            logging.error(
                f"Error initializing neural table extraction models: {str(e)}"
            )
            self.detection_model = None
            self.structure_model = None
    ...
    def _recognize_table_structure(self, image_path: str, table: NeuralTable) -> None:
        ...
        if cells:
            ...
            for cell_box, confidence in cells:
                ...
                cell_box_norm = [cell_box[0], cell_box[1], cell_box[2], cell_box[3]]
                ...
                cell = NeuralTableCell(
                    row=row, col=col, bbox=cell_box_norm, confidence=confidence
                )
    ...
    def _fallback_extraction(self, image_path: str, page_num: int) -> List[NeuralTable]:
        ...
        try:
            ...
            for i, contour in enumerate(contours):
                ...
                # Create table object
                table = NeuralTable(
                    bbox=bbox, page=page_num, confidence=0.8, structure_type="bordered"
                )
    ...
    def _detect_cells_cv(
        self, img, table: NeuralTable, x: int, y: int, w: int, h: int
    ) -> None:
        """
        ...
        try:
            ...
            contours, _ = cv2.findContours(
                joints, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE
            )
    ...
    def _detect_borderless_tables_cv(self, img, page_num: int) -> List[NeuralTable]:
        ...
        column_groups = {
            x: blocks for x, blocks in column_groups.items() if len(blocks) > 1
        }
        ...
        for i in range(len(row_centers)):
            ...
            for j in range(len(column_centers)):
                ...
                next_col_x = (
                    column_centers[j + 1] if j + 1 < len(column_centers) else max_x
                )
                ...
                for block in all_blocks:
                    ...
                    if (
                        abs(block_center_y - row_y) < y_tolerance
                        and abs(block_center_x - col_x) < x_tolerance
                    ):
