...
class TableTransformerEngine:
    ...
    def _initialize_table_transformer(self) -> bool:
        ...
        if not config.is_feature_enabled("table_transformer"):
            self.logger.info(
                "Table Transformer feature is disabled. Use config.enable_feature('table_transformer') to enable."
            )
            return False

        if not config.is_feature_available("table_transformer"):
            self.logger.warning(
                "Table Transformer dependencies not available. Install with: pip install od-parse[table_transformer]"
            )
            return False
        ...
        **kwargs,
        ...
        try:
            ...
            if self._is_available:
                return self._extract_with_table_transformer(
                    pil_image, confidence_threshold, **kwargs
                )
            else:
            ...
            return {"tables": [], "confidence": 0.0, "engine": "error", "error": str(e)}
    ...
    def _extract_with_table_transformer(
        self, image: Image.Image, confidence_threshold: float, **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_confidence = (
                sum(t["confidence"] for t in extracted_tables) / len(extracted_tables)
                if extracted_tables
                else 0
            )
    ...
    def _extract_table_structure(self, table_image: Image.Image) -> Dict[str, Any]:
        ...
        return {
            ...
            "structure_confidence": (
                float(torch.mean(results["scores"]).cpu())
    ...
    def _extract_with_fallback(
        self, image: Union[str, Path, Image.Image], **kwargs
    ) -> Dict[str, Any]:
    ...
    def visualize_detections(
        self, image: Union[str, Path, Image.Image], detections: List[Dict[str, Any]]
    ) -> Image.Image:
        """
    ...
    def get_engine_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "detection_model": (
                self.detection_model_name if self._is_available else None
            ),
            "structure_model": (
                self.structure_model_name if self._is_available else None
            ),
