...
class FormUnderstanding:
        ...
        if not checkboxes and "text_blocks" in pdf_data:
            for block in pdf_data.get("text_blocks", []):
                ...
                if (
                    ...
                    is_checked = (
                        "☑" in text or "✓" in text or "[x]" in text or "[X]" in text
                    )
        ...
        if "annotations" in pdf_data:
            ...
            for annot in pdf_data.get("annotations", []):
                if (
                    annot.get("subtype") == "Widget"
                    and annot.get("field_type") == "Btn"
                    and annot.get("radio", False)
                ):
                    # This is likely a radio button
                    x1, y1, x2, y2 = annot.get("rect", [0, 0, 0, 0])
                    page = annot.get("page", 0)
                    field_name = annot.get(
                        "field_name", f"unnamed_group_{field_id_counter}"
                    )
            ...
            for group_name, buttons in radio_groups.items():
                for i, button in enumerate(buttons):
                    radio = FormField(
                        ...
                        options=[b["option_value"] for b in buttons]
                        if all("option_value" in b for b in buttons)
                        else None,
                        confidence=0.9,
                    )
    ...
    def _detect_text_fields(self, pdf_data: Dict) -> List[FormField]:
        ...
        if "annotations" in pdf_data:
            for annot in pdf_data.get("annotations", []):
                if annot.get("subtype") == "Widget" and annot.get("field_type") in [
                    "Tx",
                    "Ch",
                ]:
                    ...
                    field_type = (
                        "text" if annot.get("field_type") == "Tx" else "dropdown"
                    )
    ...
    def _detect_signatures(self, pdf_data: Dict) -> List[FormField]:
        ...
        if "text_blocks" in pdf_data:
            for block in pdf_data.get("text_blocks", []):
                ...
                if ("signature" in text or "sign here" in text or "sign" in text) and (
                    "x" in text or "_" in text
                ):
                    x1, y1, x2, y2 = block.get("bbox", [0, 0, 0, 0])
                    page = block.get("page", 0)
    ...
    def _match_fields_with_labels(
        self, fields: List[FormField], pdf_data: Dict
    ) -> List[FormField]:
        """
        ...
        for field in fields:
            ...
            if "text_blocks" in pdf_data:
                ...
                for block in pdf_data.get("text_blocks", []):
                    ...
                    block_x1, block_y1, block_x2, block_y2 = block.get(
                        "bbox", [0, 0, 0, 0]
                    )
                    ...
                    if is_left or is_above:
                        ...
                        distance = (
                            (field_center_x - block_center_x) ** 2
                            + (field_center_y - block_center_y) ** 2
                        ) ** 0.5

                        # Update closest text if this is closer
                        if (
                            distance < min_distance and distance < 0.2
                        ):  # Within 20% of the page size
