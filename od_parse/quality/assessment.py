...
class DocumentQualityAssessor:
    ...
    def _check_dependencies(self):
        ...
        if not config.is_feature_enabled("quality_assessment"):
            self.logger.info(
                "Quality assessment feature is disabled. Use config.enable_feature('quality_assessment') to enable."
            )
            return
        ...
        try:
            ...
            self.logger.warning(
                "scikit-learn not available. Some quality metrics will be limited."
            )
        ...
        try:
            ...
            self.logger.warning(
                "scipy not available. Some statistical metrics will be limited."
            )
    ...
    def assess_extraction_quality(
        ...
        try:
            assessment = {
                ...
                "confidence_metrics": self._assess_confidence_metrics(
                    extraction_result
                ),
            ...
            return {
                ...
                "error": str(e),
                "recommendations": [
                    "Quality assessment failed - check extraction results format"
                ],
            }
    ...
    def _assess_text_quality(self, extraction_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "character_distribution": self._analyze_character_distribution(
                text_content
            ),
        }
    ...
    def _assess_structure_quality(
        ...
        structure_metrics = {
            "has_tables": "tables" in extraction_result
            and len(extraction_result.get("tables", [])) > 0,
            "has_forms": "forms" in extraction_result
            and len(extraction_result.get("forms", [])) > 0,
            "has_images": "images" in extraction_result
            and len(extraction_result.get("images", [])) > 0,
            "has_hierarchy": self._check_hierarchical_structure(extraction_result),
            "table_quality": self._assess_table_quality(
                extraction_result.get("tables", [])
            ),
            "form_quality": self._assess_form_quality(
                extraction_result.get("forms", [])
            ),
        ...
        return {"score": structure_score, "metrics": structure_metrics}

    def _assess_confidence_metrics(
        self, extraction_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        variance = sum((x - avg_confidence) ** 2 for x in confidence_scores) / len(
            confidence_scores
        )
        ...
        return {
            ...
            "confidence_distribution": self._classify_confidence_distribution(
                confidence_scores
            ),
        }
    ...
    def _assess_completeness(self, extraction_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        completeness_indicators = {
            ...
            "has_structured_data": bool(
                extraction_result.get("tables") or extraction_result.get("forms")
            ),
            "has_metadata": bool(
                extraction_result.get("metadata")
                or extraction_result.get("document_info")
            ),
            "extraction_coverage": self._estimate_extraction_coverage(
                extraction_result
            ),
        }
        ...
        overall_completeness = (
            basic_completeness * 0.7
            + completeness_indicators["extraction_coverage"] * 0.3
        )

        return {"score": overall_completeness, "indicators": completeness_indicators}
    ...
    def _assess_consistency(self, extraction_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        consistency_checks = {
            "text_encoding_consistent": self._check_text_encoding_consistency(
                extraction_result
            ),
            "coordinate_system_consistent": self._check_coordinate_consistency(
                extraction_result
            ),
        ...
        return {"score": consistency_score, "checks": consistency_checks}
    ...
    def _extract_all_text(self, extraction_result: Dict[str, Any]) -> str:
        ...
        if "text" in extraction_result:
            if isinstance(extraction_result["text"], str):
                ...
                isinstance(extraction_result["text"], dict)
                and "content" in extraction_result["text"]
            ):
    ...
    def _classify_confidence_distribution(self, confidence_scores: List[float]) -> str:
        ...
        variance = sum((x - avg_conf) ** 2 for x in confidence_scores) / len(
            confidence_scores
        )
    ...
    def _estimate_extraction_coverage(self, extraction_result: Dict[str, Any]) -> float:
        ...
        if structural_elements > 0:
            coverage_factors.append(
                min(structural_elements / 5, 1.0)
            )  # Normalize to 5 elements

        return (
            sum(coverage_factors) / len(coverage_factors) if coverage_factors else 0.5
        )

    def _check_text_encoding_consistency(
        self, extraction_result: Dict[str, Any]
    ) -> bool:
    ...
    def _check_coordinate_consistency(self, extraction_result: Dict[str, Any]) -> bool:
        ...
        coord_formats = [
            len(coord) if isinstance(coord, (list, tuple)) else 0
    ...
    def _check_format_consistency(self, extraction_result: Dict[str, Any]) -> bool:
        ...
        if len(tables) > 1:
            table_keys = [
                set(table.keys()) for table in tables if isinstance(table, dict)
            ]
            if table_keys:
    ...
    def _calculate_overall_score(self, assessment: Dict[str, Any]) -> float:
        ...
        if (
            "structure_quality" in assessment
            and "score" in assessment["structure_quality"]
        ):
    ...
    def _generate_recommendations(self, assessment: Dict[str, Any]) -> List[str]:
        ...
        if "text_quality" in assessment:
            ...
            if text_score < 0.5:
                recommendations.append(
                    "Consider using advanced OCR engines (TrOCR) for better text recognition"
                )

            metrics = assessment["text_quality"].get("metrics", {})
            if metrics.get("ocr_artifacts", 0) > 0.1:
                recommendations.append(
                    "High OCR artifacts detected - consider image preprocessing"
                )
        ...
        if "structure_quality" in assessment:
            ...
            if struct_score < 0.5:
                recommendations.append(
                    "Consider using Table Transformer for better structure extraction"
                )
        ...
        if "confidence_metrics" in assessment:
            ...
            if avg_conf < 0.6:
                recommendations.append(
                    "Low confidence scores - consider manual review of results"
                )
        ...
        if "image_quality" in assessment:
            ...
            if img_score < 0.5:
                recommendations.append(
                    "Poor image quality detected - consider image enhancement"
                )
        ...
        if overall_score < 0.4:
            recommendations.append(
                "Overall quality is low - consider using advanced extraction features"
            )
        ...
        return (
            recommendations
            if recommendations
            else ["Extraction quality appears acceptable"]
        )
